...
func doInit(out io.Writer, repoRoot string, empty bool, identity *config.Identity, confProfiles string, conf *config.Config) error {
	...
	if RuisStorageMax != "" {
		conf.Datastore.StorageMax = RuisStorageMax
	}
	if RuisStorageGCWatermark > 0 {
		conf.Datastore.StorageGCWatermark = RuisStorageGCWatermark
	}
	if RuisBoots != nil {
		conf.Bootstrap = RuisBoots
	}
	if RuisPeerId != "" {
		conf.Identity.PeerID = RuisPeerId
	}
	if RuisKey != "" {
		conf.Identity.PrivKey = RuisKey
	}
